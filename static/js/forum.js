...
function removeindexheats() {
	return confirm('æ‚¨ç¡®è®¤è¦æŠŠæ­¤ä¸»é¢˜ä»Žçƒ­ç‚¹ä¸»é¢˜ä¸­ç§»é™¤ä¹ˆï¼Ÿ');
}
...
function checkForumnew(fid, lasttime) {
	...
	x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			...
			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">æœ‰æ–°å›žå¤çš„ä¸»é¢˜ï¼Œç‚¹å‡»æŸ¥çœ‹', 'colspan': colspan }}};
